...
public class VerticalTabLayout extends ScrollView {
     ...
     * VerticalTabLayout's content height. Fixed tabs have equal height, based on the highest tab
     * label. Once the tabs outgrow the view's height, auto-sizing tabs behave like MODE_SCROLLABLE,
     * allowing for a dynamic number of tabs without requiring additional layout logic.
     *
     * @see #setTabMode(int)
     * @see #getTabMode()
     */
    ...
    /**
     * If a tab is instantiated with {@link Tab#setText(CharSequence)}, and this mode is set, the
     * text will be saved and utilized for the content description, but no visible labels will be
     * created.
     *
    ...
    /**
     * This mode is set by default. If a tab is instantiated with {@link Tab#setText(CharSequence)},
     * a visible label will be created.
     *
     * @see Tab#setTabLabelVisibility(int)
     */
    ...
    /**
     * Gravity used to fill the {@link VerticalTabLayout} as much as possible. This option only
     * takes effect when used with {@link #MODE_FIXED} on non-landscape screens less than 600dp
     * wide.
     *
     ...
     * VerticalTabLayout}. This will only take effect if the indicator width is set via the custom
     * indicator drawable's intrinsic width (preferred), the indicator will not be shown.
     *
     * @attr ref com.google.android.material.R.styleable#TabLayout_tabIndicatorGravity
     * @see #setSelectedTabIndicatorGravity(int)
     * @see #getTabIndicatorGravity()
     ...
     * width of the {@link VerticalTabLayout}. This will disregard {@code tabIndicatorHeight} and
     * the indicator drawable's intrinsic width, if set.
     *
     * @attr ref com.google.android.material.R.styleable#TabLayout_tabIndicatorGravity
     * @see #setSelectedTabIndicatorGravity(int)
     * @see #getTabIndicatorGravity()
    ...
    public VerticalTabLayout(@NonNull Context context,
        ...
        tabPaddingStart = tabPaddingTop = tabPaddingEnd = tabPaddingBottom = a.getDimensionPixelSize(R.styleable.TabLayout_tabPadding, 0);
        ...
        // CHANGE
        requestedTabMinHeight = a.getDimensionPixelSize(R.styleable.TabLayout_tabMinWidth, INVALID_HEIGHT);
        requestedTabMaxHeight = a.getDimensionPixelSize(R.styleable.TabLayout_tabMaxWidth, INVALID_HEIGHT);
     ...
     *
     * <p>Calling this method does not update the selected tab, it is only used for drawing
     * purposes.
     *
    ...
    /**
     * Set the indicator gravity used to align the tab selection indicator in the {@link
     * VerticalTabLayout}. You must set the indicator height via the custom indicator drawable's
     * intrinsic height (preferred). Otherwise, the indicator will not be shown unless gravity is
     * set to {@link #INDICATOR_GRAVITY_STRETCH}, in which case it will ignore indicator height and
     * stretch across the entire height and width of the {@link VerticalTabLayout}. This defaults to
     * {@link #INDICATOR_GRAVITY_LEFT} if not set.
     *
     ...
     *
     * <p>Defaults to true. If set to false and the tab item has a text label, the selection
     * indicator width will be set to the width of the text label. If the tab item has no text
     * label, but does have an icon, the selection indicator width will be set to the icon. If the
     * tab item has neither of these, or if the calculated width is less than a minimum width value,
     * the selection indicator width will be set to the minimum width value.
     *
    ...
    /**
     * Set whether this {@link VerticalTabLayout} will have an unbounded ripple effect or if ripple
     * will be bound to the tab item size.
     *
     * <p>Defaults to false.
     *
    ...
    /**
     * Set whether this {@link VerticalTabLayout} will have an unbounded ripple effect or if ripple
     * will be bound to the tab item size. Defaults to false.
     *
    ...
    /**
     * Returns whether this {@link VerticalTabLayout} has an unbounded ripple effect, or if ripple
     * is bound to the tab item size.
     *
     * @attr ref com.google.android.material.R.styleable#TabLayout_tabUnboundedRipple
     ...
     *
     * <p>When running on devices with KitKat or below, we draw this color as a filled overlay
    ...
    /**
     * Sets the selection indicator for this VerticalTabLayout. By default, this is a line along the
     * bottom of the tab. If {@code tabIndicatorColor} is specified via the VerticalTabLayout's
     * style or via {@link #setSelectedTabIndicatorColor(int)} the selection indicator will be
     * tinted that color. Otherwise, it will use the colors specified in the drawable.
     *
    ...
    /**
     * Sets the drawable resource to use as the selection indicator for this VerticalTabLayout. By
     * default, this is a line along the bottom of the tab. If {@code tabIndicatorColor} is
     * specified via the VerticalTabLayout's style or via {@link #setSelectedTabIndicatorColor(int)}
     * the selection indicator will be tinted that color. Otherwise, it will use the colors
     * specified in the drawable.
     *
     * @param tabSelectedIndicatorResourceId A drawable resource to use as the selected tab
     *     indicator.
     * @see #setSelectedTabIndicatorColor(int)
     ...
     * The one-stop shop for setting up this {@link VerticalTabLayout} with a {@link ViewPager}.
     *
     * <p>This method will link the given ViewPager and this VerticalTabLayout together so that
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (MeasureSpec.getMode(heightMeasureSpec) != MeasureSpec.UNSPECIFIED) {
            ...
            // the max tab width
            tabMaxHeight = requestedTabMaxHeight > 0
    ...
    public void selectTab(@Nullable final Tab tab, boolean updateIndicator) {
        ...
        if (currentTab == tab) {
            ...
            // Setting selectedTab before dispatching 'tab unselected' events, so that currentTab's
            // state
    ...
    public static class Tab {
        ...
        // This represents the content description that has been explicitly set on the Tab or
        // TabItem
        // in XML or through #setContentDescription. If the content description is empty, text
        // should
        ...
        @Nullable public VerticalTabLayout parent;
        public TabView view;
         ...
         *
         * <p>If the provided view contains a {@link TextView} with an ID of {@link
        ...
        public Tab setText(@Nullable CharSequence text) {
            if (TextUtils.isEmpty(this.contentDesc) && !TextUtils.isEmpty(text)) {
                // If no content description has been set, use the text as the content description
                // of the
         ...
         *
         * <ul>
         *   <li>{@link #TAB_LABEL_VISIBILITY_UNLABELED}: Tabs will appear without labels regardless
         *       of whether text is set.
    ...
    /**
     * A {@link LinearLayout} containing {@link Tab} instances for use with {@link
     * VerticalTabLayout}.
     */
    public final class TabView extends LinearLayout {
        ...
        /**
         * Draw the background drawable specified by tabBackground attribute onto the canvas
         * provided. This method will draw the background to the full bounds of this TabView. We
         * provide a separate method for drawing this background rather than just setting this
         * background on the TabView so that we can control when this background gets drawn. This
         * allows us to draw the tab background underneath the TabLayout selection indicator, and
         * then draw the TabLayout content (icons + labels) on top of the selection indicator.
         *
        ...
        final void update() {
            ...
            if (customView == null) {
                ...
                final Drawable icon = (tab != null && tab.getIcon() != null)
         ...
         *
         * @return Width of the tab label, if present, or the width of the tab icon, if present. If
         *     tabs is in inline mode, returns the sum of both the icon and tab label widths.
         */
    ...
    private class SlidingTabIndicator extends LinearLayout {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            // GRAVITY_CENTER will make all tabs the same height as the largest tab, and center them
            // in the
            // SlidingTabIndicator's height (with a "gutter" of padding on either side). If the Tabs
            // do not
            ...
            if (tabGravity == GRAVITY_CENTER || mode == MODE_AUTO) {
                ...
                if (largestTabHeight * count <= getMeasuredHeight() - gutter * 2) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) getChildAt(i).getLayoutParams();
         ...
         * present, calculates the width of the icon. If both are present, the text label bounds
         * take precedence. If both are present and inline mode is enabled, the sum of the bounds of
         * the both the text label and icon are calculated. If neither are present or if the
         * calculated difference between the left and right bounds is less than 24dp, then left and
         * right bounds are adjusted such that the difference between them is equal to 24dp.
         *
        ...
        public void draw(@NonNull Canvas canvas) {
            ...
            // Draw the tab item contents (icon and label) on top of the background + indicator
            // layers
            super.draw(canvas);
        }
    }
    ...
    public static class PageChangeListener implements ViewPager.OnPageChangeListener {
        @NonNull private final WeakReference<VerticalTabLayout> tabLayoutRef;
